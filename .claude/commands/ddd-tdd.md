...
依照 `docs/agent-workflow-ddd-tdd.md` 完整執行 **Phase 0 → 6**：

- Phase 0/1/6 內容直接寫在主索引
- Phase 2~5 內容在 `docs/workflow/phase-X-*.md`，進入該 Phase 時才需載入對應檔案

## 執行守則（不寫在主索引、僅此處）

- 每個 Phase 結束**回報進度並等待使用者確認**，不自動跳到下一 Phase
- Phase 2 的 **RED 階段**必須實際執行 `./gradlew test` 確認 FAIL（原因為「實作不存在」而非 syntax error）；**GREEN 階段**必須實際執行確認 PASS
